...
When no client entry points are specified via `environments.client.build.rolldownOptions.input` in the Vite configuration, the plugin assumes a server-only application and puts the Fastly server proxy _before_ Vite's client assets server in the middleware stack. This setup essentially disables most client-specific Vite features.
